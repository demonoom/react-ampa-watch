...
export default class watchTrail extends React.Component {
    ...
    buildLocationName = (data) => {
        ...
        data.forEach((v, i) => {
            ...
            if (i == 0) {
                locationName.push(
                    <Step icon={<img className="StepStart" src={require('../img/icon-startingPoint.png')} alt=""/>} title={v.locationName} description={WebServiceUtil.formatHM(v.time)}/>
                )
            } else if (i == data.length - 1) {
                locationName.push(
                    <Step icon={<img className="StepEnd" src={require('../img/icon-end.png')} alt=""/>} title={v.locationName} description={WebServiceUtil.formatHM(v.time)}/>
                )
            } else {
                locationName.push(
                    <Step icon={<img className="StepDot" src={require('../img/icon-dot.png')} alt=""/>} title={v.locationName} description={WebServiceUtil.formatHM(v.time)}/>
                )
            }
        });
